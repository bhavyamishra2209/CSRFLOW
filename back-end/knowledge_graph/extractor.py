...
class EnhancedEntityExtractor:
    ...
    def load_spacy(self, model_name: str) -> None:
        ...
        """
        if spacy is None:
            logger.warning("spaCy module not installed; using rule-based regex entity extraction.")
            self.nlp = None
            self.use_ner = False
            return
    ...
    def extract_entities(self, text: str) -> List[Tuple[str, str, float]]:
        ...
        if self.nlp is not None:
            ...
            # Fallback rule-based entity extraction when spaCy is not available
            rule_patterns = [
                (r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,2})\b', 'Person'),
                (r'\b([A-Z][A-Za-z0-9\s&]+(?:Trust|Foundation|Society|NGO|Association|Ltd|Limited|Corp|Corporation))\b', 'Organization'),
                (r'\b(Q[1-4]\s*(?:FY\s*)?\d{4}[–\-]\d{2,4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})\b', 'Date'),
                (r'\b(?:INR|Rs\.?|₹|■)\s*([\d,]+(?:\.\d{1,2})?)\b', 'Money'),
            ]
            for pat, etype in rule_patterns:
                for m in re.finditer(pat, text):
                    val = m.group(1).strip()
                    if val and len(val) > 2:
                        entities.append((val, etype, 0.8))
