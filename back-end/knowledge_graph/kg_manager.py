...
class KGManager:
    ...
    _FIELD_TYPE_MAP: Dict[str, str] = {
        # CSR Enterprise Document Fields
        "project_title":             "Project",
        "implementing_agency":        "Organization",
        "organization":               "Organization",
        "company_name":               "Organization",
        "partner_organization":       "Organization",
        "issuing_authority":          "Organization",
        "chartered_accountant_name": "Person",
        "ca_name":                    "Person",
        "contact_person":             "Person",
        "authorized_signatory":       "Person",
        "csr_committee_members":      "Person",
        "reporting_period":           "Date",
        "completion_date":            "Date",
        "submission_date":            "Date",
        "approval_date":              "Date",
        "certification_date":         "Date",
        "funds_utilized":             "FinancialValue",
        "total_budget":               "FinancialValue",
        "allocated_budget":           "FinancialValue",
        "spent_amount":               "FinancialValue",
        "certified_amount":           "FinancialValue",
        "final_cost":                 "FinancialValue",
        "funding_amount":             "FinancialValue",
        "uc_number":                  "Identifier",
        "proposal_reference":         "Identifier",
        "approval_reference":         "Identifier",
        "financial_year":             "TimePeriod",
        "beneficiary_count":          "Metric",
        "target_beneficiaries":       "Metric",

        # Legacy ID Card Fields
        "full_name":                  "Person",
    ...
    def process_extracted_fields(
        ...
        try:
            ...
            # ── CSR Enterprise Semantic Edge Construction ────────────────────
            proj_id = field_node_ids.get("project_title")
            org_id = field_node_ids.get("implementing_agency") or field_node_ids.get("organization") or field_node_ids.get("company_name")

            if proj_id and org_id:
                ekg.add_relation(proj_id, "IMPLEMENTED_BY", org_id, 0.95)

            for fin_field in ("funds_utilized", "total_budget", "allocated_budget", "spent_amount", "certified_amount", "final_cost"):
                fin_id = field_node_ids.get(fin_field)
                if fin_id and proj_id:
                    ekg.add_relation(proj_id, "HAS_FINANCIAL_VALUE", fin_id, 0.9)
                elif fin_id and doc_node_id:
                    ekg.add_relation(doc_node_id, "CONTAINS_AMOUNT", fin_id, 0.85)

            for date_field in ("reporting_period", "completion_date", "submission_date", "approval_date"):
                date_id = field_node_ids.get(date_field)
                if date_id and proj_id:
                    ekg.add_relation(proj_id, "REPORTED_PERIOD", date_id, 0.9)

            for person_field in ("chartered_accountant_name", "ca_name", "contact_person", "authorized_signatory", "csr_committee_members"):
                p_id = field_node_ids.get(person_field)
                if p_id and org_id:
                    ekg.add_relation(p_id, "MEMBER_OF", org_id, 0.9)
                elif p_id and doc_node_id:
                    ekg.add_relation(p_id, "SIGNED_DOCUMENT", doc_node_id, 0.9)
