...
class LocalLLM(BaseLLM):
    ...
    def _generate_answer(self, chunks: List[str], question: str) -> str:
        ...
        # Clean up question - remove quotes and leading/trailing punctuation
        question = question.strip().strip('"').strip("'").strip()
        question_lower = question.lower()
        
        # Extract key terms from the question (remove stop words and punctuation)
        stop_words = {'what', 'is', 'the', 'a', 'an', 'are', 'how', 'where', 'when', 'who', 'why', 'which', 'do', 'does', 'can', 'could', 'would', 'should', 'for', 'of', 'in', 'on', 'at', 'to'}
        raw_words = [re.sub(r'[^a-zA-Z0-9]', '', w.lower()) for w in question.split()]
        question_words = [w for w in raw_words if w and w not in stop_words and len(w) > 2]
