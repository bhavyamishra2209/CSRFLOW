...
try:
    ...
    if VECTOR_STORE_DIR.exists():
        try:
            vector_db = FaissVectorDatabase.load(str(VECTOR_STORE_DIR))
            logger.info("✓ Vector database loaded from disk store")
        except Exception as _vdb_err:
            logger.warning(f"Failed to load vector database from disk: {_vdb_err}")
...
try:
    ...
    # Auditor / Approver Role-Based Control routes
    try:
        from routes.auditor_routes import register_auditor_routes
        register_auditor_routes(app)
        logger.info("✓ Auditor RBAC routes registered")
    except Exception as _are:
        logger.warning(f"Auditor routes not registered: {_are}")

    # Enterprise 3-Role RBAC routes (CSR Head, PM, Auditor)
    try:
        from routes.rbac_routes import register_rbac_routes
        register_rbac_routes(app)
        logger.info("✓ Enterprise 3-Role RBAC routes registered")
    except Exception as _rbe:
        logger.warning(f"3-Role RBAC routes not registered: {_rbe}")
