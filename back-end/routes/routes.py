...
_DOC_REGISTRY_FILE = Path(__file__).parent.parent / "doc_store" / "doc_registry.json"


def _save_doc_registry() -> None:
    try:
        _DOC_REGISTRY_FILE.parent.mkdir(parents=True, exist_ok=True)
        with open(_DOC_REGISTRY_FILE, "w", encoding="utf-8") as f:
            json.dump(_DOC_REGISTRY, f, indent=2, default=str)
    except Exception as e:
        logger.warning(f"Could not save document registry to disk: {e}")


def _load_doc_registry() -> None:
    global _DOC_REGISTRY
    if _DOC_REGISTRY_FILE.exists():
        try:
            content = _DOC_REGISTRY_FILE.read_text(encoding="utf-8").strip()
            if content:
                loaded = json.loads(content)
                if isinstance(loaded, dict):
                    _DOC_REGISTRY.update(loaded)
                    logger.info(f"Loaded {len(loaded)} documents into _DOC_REGISTRY from disk")
        except Exception as e:
            logger.warning(f"Could not load document registry from disk: {e}")
...
class RAGAPIRouter:
    ...
    def _register_routes(self):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        self.rag_engine.vector_db.save(str(vector_dir))
                        logger.info("✓ Vector database saved to disk after upload")
                    except Exception as _vdb_save_err:
                        logger.warning(f"Could not auto-save vector database: {_vdb_save_err}")
                    _fb_update({
                        "status": "INDEXING_KG",
        ...
        async def query(
            ...
            try:
                ...
                try:
                    ...
                    pass
                # Scope search: PMs search their docs; Head/Admin & Auditor search across enterprise docs
                user_filter = dict(query_input.filter_dict or {})
                if user.role.lower() not in ("csr_head", "admin", "auditor", "csr head"):
                ...
                if query_input.use_kg and KG_AVAILABLE:
                    try:
                        clean_query_str = query_input.query.replace('"', '').replace("'", "").strip()
                        words = [
                            w.strip("?,.!\"':;")
                            for w in clean_query_str.split()
                            if len(w) > 2
                        ]
                        hits = []
                        seen = set()
                        for word in words:
                            ...
                            if res.get("found") and res["name"] not in seen:
                                seen.add(res["name"])
                                hits.append(res)
                        total_rels = sum(len(h.get("connections", [])) for h in hits)
                        kg_context = {
                            "matched_entities": hits,
                            "entities_used": len(hits),
                            "relations_used": total_rels,
                ...
                if ("couldn't find" in final_response.lower() or "don't have enough" in final_response.lower()) and kg_context and kg_context.get("matched_entities"):
                    lines = ["Based on the Knowledge Graph entity network:"]
                    for ent in kg_context["matched_entities"]:
                        lines.append(f"- **{ent.get('name')}** ({ent.get('type')})")
                        for conn in ent.get("connections", [])[:4]:
                            lines.append(f"   -> [{conn.get('relation')}] -> **{conn.get('entity_name')}** ({conn.get('entity_type')})")
                    final_response = "\n".join(lines)
        ...
        @self.app.get("/documents", summary="List all processed documents (paginated)")
        @self.app.get(
            "/documents/list",
        ...
        async def list_documents(
            ...
            """
            is_admin = user.role.lower() in ("csr_head", "admin", "auditor", "csr head")
            docs = [
                d for d in _DOC_REGISTRY.values()
                if is_admin or d.get("owner_id") == user.user_id
            ]

            # Enrich from FAISS for chunks uploaded before the registry existed
            seen_ids = {d["document_id"] for d in docs if "document_id" in d}
            vdb = getattr(self.rag_engine, "vector_db", None)
            if vdb and hasattr(vdb, "documents"):
                for chunk in vdb.documents.values():
                    did  = chunk.metadata.get("document_id")
                    oid  = chunk.metadata.get("owner_id", "")
                    if did and did not in seen_ids and (is_admin or oid == user.user_id):
                        seen_ids.add(did)
                        docs.append({
                            "document_id":               did,
        ...
        async def get_document(
            ...
            # Wrong owner → treat as 404 (don't leak existence) unless admin role
            is_admin = user.role.lower() in ("csr_head", "admin", "auditor", "csr head")
            if not is_admin and rec.get("owner_id") != user.user_id:
                raise HTTPException(
                    status_code=404,
                    detail={
                        "error":       "Document not found",
        ...
        async def delete_document(
            ...
            # Ownership check — 404 (not 403) so we don't leak existence unless admin role
            is_admin = user.role.lower() in ("csr_head", "admin", "auditor", "csr head")
            rec = _DOC_REGISTRY.get(document_id, {})
            if not is_admin:
        ...
        class VerifyDocumentRequest(BaseModel):
            status: Optional[str] = Field(None, description="Explicit verification decision: 'verified', 'rejected', 'needs_review'")
            reason: Optional[str] = Field(None, description="Auditor notes or review explanation")

        @self.app.post(
            "/documents/{document_id}/verify",
            summary="Verify document against records database or set Auditor verification decision",
        )
        async def verify_document_endpoint(
            ...
            """
            Runs trust-score verification or stores explicit Auditor decision.
            """
            rec = _DOC_REGISTRY.get(document_id)
            if rec is None:
                vdb = getattr(self.rag_engine, "vector_db", None)
                if vdb and hasattr(vdb, "documents"):
                    ...
                    if chunks:
                        ...
                        _DOC_REGISTRY[document_id] = rec
                        _save_doc_registry()

            is_admin = user.role.lower() in ("csr_head", "admin", "auditor", "csr head")
            if rec is None or (not is_admin and rec.get("owner_id") != user.user_id):
                raise HTTPException(
                    status_code=404,
                    detail={"error": "Document not found", "document_id": document_id},
                )

            # Explicit Auditor verification decision override
            if payload and payload.status:
                v_status = payload.status.lower()
                v_reason = payload.reason or f"Auditor ({user.email}) decision: {v_status.title()}"
                v_conf = 1.0 if v_status in ("verified", "complete") else 0.5
                rec["verification_status"] = v_status
                rec["verification_confidence"] = v_conf
                rec["verification_reason"] = v_reason
                rec["reason"] = v_reason
                _save_doc_registry()
                logger.info(f"Auditor {user.email} set document {document_id} status to {v_status}")
                return {
                    "document_id": document_id,
