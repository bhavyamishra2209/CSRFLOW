...
async def user_stats(user: UserInfo = Depends(get_current_user)):
    ...
    # Average OCR confidence (only docs that went through OCR)
    ocr_scores = [
        rec["ocr_confidence"]
        for rec in user_docs
        if rec.get("ocr_confidence") is not None
    ]
    avg_ocr = round(sum(ocr_scores) / len(ocr_scores), 4) if ocr_scores else None
    ...
    return {
        ...
        "total_queries_made":           _USER_QUERY_COUNTERS.get(user.user_id, 0),
    }
