...
class OCRProcessor:
    ...
    def __init__(self, engine: Optional[OCREngine] = None):
        ...
        """
        if engine is None:
            # Try Tesseract binary first; fall back to EasyOCR if not installed
            try:
                import pytesseract as _pt
                _pt.get_tesseract_version()  # raises if binary missing
                self.engine = TesseractOCR()
                logger.info("OCRProcessor using Tesseract")
            except Exception:
                logger.warning("Tesseract binary not found, falling back to EasyOCR")
                try:
                    self.engine = EasyOCREngine()
                    logger.info("OCRProcessor using EasyOCR")
                except ImportError:
                    raise RuntimeError(
...
def extract_text_from_image(image: Image.Image) -> tuple[str, float]:
    ...
    Tries Tesseract first; if the binary is missing falls back to EasyOCR.
    ...
    try:
        engine = TesseractOCR()
        # Quick sanity check — will raise if binary missing
        import pytesseract as _pt
        ...
        logger.warning("Tesseract returned empty text, trying EasyOCR")
    except Exception as e:
        logger.warning(f"Tesseract not available ({e}), falling back to EasyOCR")
